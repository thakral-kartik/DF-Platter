...
def eval_frames_setBC(fnames, targets, preds, save_name):
    
    # print(fnames[:15], len(fnames))
    ...
    for i, f in enumerate(fnames):
        ...
        new_targets[ind].append(int(targets[i]))
        new_scores[ind].append(preds[i, 1]) 
        new_preds[ind].append(np.argmax(preds[i], axis=0))

    # correct = 0
    # for i in range(len(new_targets)):
    #     # if new_targets[i] == new_preds[i]:
    #     # print(new_targets[i], new_preds[i])
    #     if np.array_equal(new_targets[i], new_preds[i], equal_nan=True):
    #         correct+=1
    # acc = correct/len(new_targets)
    # print("Frame-wise Accuracy: ", acc)

    # return correct, len(new_targets), acc
...
# dfname = fnames[0]
# print(get_video_name_setA(dfname))
# quit()

print("Set: ", set_name_eval, " resolution: ", resolution, " compression: ", compression, " model name:", model_name, "\nTested on:", test_set_name, "\n")

# # For Set A
print("\n Frame-wise performance:")
...
# fnames = df['filepath']
# targets = df['labels']
# preds = np.random.uniform(low=0, high=1, size=(targets.shape[0],2))
# mat_name = "dummy.mat"

# for fname in fnames:
#     print(fname, get_frame_name_setBC(fname))
#     print(fname, get_video_name_setBC(fname))
