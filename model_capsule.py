...
class RoutingLayer(nn.Module):
    ...
    def forward(self, x, random, dropout):
        ...
        if random:
            ...
            if self.gpu_id >= 0:
                # noise = noise.to('cuda:'+str(self.gpu_id))
                # noise = noise.cuda(self.gpu_id)
                noise = torch.nn.DataParallel(noise, device_ids=[0,3])
        ...
        if dropout > 0.0:
            ...
            if self.gpu_id >= 0:
                # drop = drop.to('cuda:'+str(self.gpu_id))
                # drop = drop.cuda(self.gpu_id)
                drop = torch.nn.DataParallel(drop, device_ids=[0,3])
                drop = drop.to(config.device)
                pass
            priors = priors * drop
        ...
        if self.gpu_id >= 0:
            # logits = logits.to('cuda:'+str(self.gpu_id))
            # logits = logits.cuda(self.gpu_id)
            logits = torch.nn.DataParallel(logits, device_ids=[0,3])
...
class CapsuleNet(nn.Module):
    def __init__(self, num_class, gpu_id):
        ...
        self.fea_ext = torch.nn.DataParallel(self.fea_ext, device_ids=[0,3]).to('cuda:'+str(gpu_id))
...
class CapsuleLoss(nn.Module):
    def __init__(self, gpu_id):
        ...
        if gpu_id >= 0:
            self.cross_entropy_loss.to('cuda:'+str(self.gpu_id))
...
if __name__=='__main__':
    ...
    a = torch.zeros([32, 3, 224, 224]).to(config.device)
    vgg = VggExtractor(train=True).to(config.device)
    op = vgg(a)
    print(op.shape)

    capsule = CapsuleNet(4, 0).to(config.device)
    op2 = capsule(op)
    print(op2.shape)